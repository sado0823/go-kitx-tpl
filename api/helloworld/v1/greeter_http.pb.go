...
const OperationGreeterGotError = "/helloworld.v1.Greeter/GotError"
...
func RegisterGreeterHTTPServer(r *http.Router, srv GreeterHTTPServer) {
	...
	r.GET("/error", _Greeter_GotError0_HTTP_Handler(srv))
}
...
func _Greeter_GotError0_HTTP_Handler(srv GreeterHTTPServer) func(ctx http.Context) error {
	return func(ctx http.Context) error {
		var in emptypb.Empty
		if err := ctx.BindQuery(&in); err != nil {
			return err
		}
		http.SetOperation(ctx, OperationGreeterGotError)
		h := ctx.Middleware(func(ctx context.Context, req interface{}) (interface{}, error) {
			return srv.GotError(ctx, req.(*emptypb.Empty))
		})
		out, err := h(ctx, &in)
		if err != nil {
			return err
...
type GreeterHTTPClient interface {
	...
	GotError(ctx context.Context, req *emptypb.Empty, opts ...http.CallOption) (rsp *emptypb.Empty, err error)
...
func (c *GreeterHTTPClientImpl) GotError(ctx context.Context, in *emptypb.Empty, opts ...http.CallOption) (*emptypb.Empty, error) {
	var out emptypb.Empty
	pattern := "/error"
	path := binding.EncodeURL(pattern, in, true)
	opts = append(opts, http.Operation(OperationGreeterGotError))
	opts = append(opts, http.PathTemplate(pattern))
